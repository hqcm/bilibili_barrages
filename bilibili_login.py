...
class Geetest(object):
    def __init__(self):
        with open('location_list.pkl', 'rb') as f:
        ...
        self.driver = webdriver.Firefox(
            executable_path=
            r'D:\Program Files (x86)\Mozilla Firefox\geckodriver')
    ...
    def get_bg_fullbg(self):
        "获取验证码的地址"
        html = self.driver.find_elements_by_class_name('gt_cut_fullbg_slice')[
            0].get_attribute('style')
        fullbg_picture = re.findall(r'url\(\"(.+)\"\)', html)[0]
        html = self.driver.find_elements_by_class_name('gt_cut_bg_slice')[
            0].get_attribute('style')
        bg_picture = re.findall(r'url\(\"(.+)\"\)', html)[0]
        fullbg_picture = Image.open(
            BytesIO((requests.get(fullbg_picture).content)))
    ...
    def get_merge_image(self, img):
        ...
        for i in range(51):
            ...
            if self.location_list[name]['y'] == 58:
                box1 = (self.location_list[name]['x'], 58,
                        self.location_list[name]['x'] + 10, 58 + 58)
                box2 = (10 * i, 0, 10 * i + 10, 58)
            #取出原图的下部放入新图的上部
            else:
                box1 = (self.location_list[name]['x'], 0,
                        self.location_list[name]['x'] + 10, 58)
    ...
    def get_track(self, x_pos):
        ...
        if track_list[-1][0] - x_pos > 3:
            track_list.append(
                [x_pos, track_list[-1][2] + random.uniform(0.1, 0.2)])
            distance_list.append([
                x_pos - track_list[-1][0],
                distance_list[-1][2] + random.randint(10, 20) / 100
            ])
        return distance_list

    def simulate_move(self, distance_list):
        slider_knob = self.driver.find_element_by_xpath(
            '//div[@class="gt_slider_knob gt_show"]')
        logger.debug('验证中...')
        # 点击滑块并按住不放
        ActionChains(
            self.driver).click_and_hold(on_element=slider_knob).perform()
        time.sleep(0.01)
        #拖动滑块
        for distance in distance_list:
            ...
            ActionChains(self.driver).move_by_offset(
                xoffset=x_offset, yoffset=0).perform()
